...
export default {
  ...
  watch: {
    clave() {
      this.jugador.clave = this.clave
    }
  },
  computed: {
    clave() {
      if (this.jugador.name) {
        return this.jugador.name.replace(/ /g, '-')
      } else {
        return null
      }
    }
  ...
  methods: {
    onSubmitButton() {
      const response = db.collection('primer_equipo_jugadores').add(this.jugador)
        response.then(() => {
         this.$router.back()
        })
    }
