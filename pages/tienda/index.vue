    ...
    <div class="col-dx">
      <div class="cart">
        ...
        <table class="cart-list" v-if="cart.length">
          <thead>
            <tr>
              <th class="head-title">Producto</th>
              <th class="head-price">Precio</th>
              <th class="head-quantity">Cantidad</th>
        ...
        <div class="empty-contents" v-else>
          ...
          <p class="no-items-text">Tu carrito está vacio</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    addItem(prod){
      ...
      // Añade el producto en el carrito
      if(! inCart){
        this.cart.push({
          id: prod.id,
          title: prod.name,
